            ...
            <tr>
                <td>
                    <table width="100%" cellpadding="0" cellspacing="1" class="formtable">
                        <tbody>
<%--                         <c:if test="${not empty routeFlowToers}">
                            <tr id="backTr">	
                        ...
                        <c:if test="${empty routeFlowToers}">
                           <input id="agreeActivitiId" type="hidden" value="" />
                        </c:if> --%>
                        <tr id="saveTr">
                            <td class="tdtitle" align="right" width="25%">
                                意见:
                        ...
                        <tr id="trPortrait">
                            ...
                            <td class="tdtext">
                                <input id="chkPortrait" type="checkbox" name="chkPortrait" checked="checked" disabled="disabled">
                            </td>
                        </tr>
                        </tbody>
                    </table>
...
<script type="text/javascript">
    ...
    function setParent(dialogId, isMail, isPhoneMessage, idea,isPortrait) {
        if (dialogId != '') {
            if (dg.iDG(dialogId)) {
            	$("#btnConfirm").val("提交中...");
            	$("#btnConfirm").attr("disabled","disabled");//添加disabled属性 
            	$("#btnCancel").attr("disabled","disabled");//添加disabled属性 
           		dg.iWin(dialogId).commitAgree(isMail, isPhoneMessage, idea,isPortrait);
                dg.cancel();
            }
            else {
                alert('请先打开父窗口');
    ...
    $('#btnConfirm').click(function () {
    	...
    	var chkPortrait = $('#chkPortrait').is(':checked') == true ? "on" : "off";
        setParent(dialogId, chkMail, chkPhoneMessage, $('#txaApprovaIdea').val(),chkPortrait);
    });
</script>
</body>
</html>
