...
//2014-7-23为下拉框输入失去焦点事件 onfocus=\"alert('获得焦点')\"  onblur=\"alert('失去焦点')\"
...
_85c.unbind(".combo");
if(!opts.disabled&&!opts.readonly){
	//2014-6-19添加汉子匹配模糊查询的方法 lkquery为新添加的属性 用来判断是否允许模糊查询
	if(opts.lkquery){
...
//2014-6-19 如果包含复选框 那么禁用下拉框row单击选择事件

if(opts.chkbox){
	//opts.finder.getRow(_8a9,_8aa).id获取当前选中行的id
	var obj = $("#"+opts.prefix+opts.finder.getRow(_8a9,_8aa).id);
	//$("**").attr("attrName");而jQuery的版本用的是1.9，这就是存在一个兼容性和稳定性问题。 
	//jQuery API明确说明，1.6+的jQuery要用prop，尤其是checkBox的checked的属性的判断
	obj.prop("checked",true);//选中
}
...
if(opts.chkbox){
   //opts.finder.getRow(_8a9,_8aa).id获取当前选中行的id
   var obj = $("#"+opts.prefix+opts.finder.getRow(_8ae,_8af).id);
   //$("**").attr("attrName");而jQuery的版本用的是1.9，这就是存在一个兼容性和稳定性问题。 
   //jQuery API明确说明，1.6+的jQuery要用prop，尤其是checkBox的checked的属性的判断
   obj.prop("checked",false);//不选中
}
...
//2014-6-19添加带复选框的下拉框  chkbox为真带复选框 为false不带  默认为false); prefix用来区分同页面下拉框id相同复选框选择在别的下拉框bug问题
var prefix = opts.prefix;
if(opts.chkbox){
	prefix = prefix+v;
	dd.push("<input type=checkbox id=\""+(prefix)+"\" value=\""+v+"\" style=margin-top:0px;>&nbsp;&nbsp;</input>");
}
