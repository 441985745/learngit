		...
		<div>
			<table class="table"  >
				<tr style="text-align: center;">
					<td><th class="font_class">资产报废变卖申请单</th></td>
				</tr>
				<tr >
					<td class="td_right">申请流程单号：</td>
					<td >${discardSell.document}</td>
					<td class="td_right">申请人：</td>
					<td >${discardSell.userCode}/${discardSell.createBy}</td>			
				</tr>
				<tr >
					<td >申请日期:</td>
					<td ><fmt:formatDate value="${discardSell.createDate}"  pattern="yyyy-MM-dd"/></td>
					<td >申请人所属部门:</td>
					<td >${discardSell.departmentName}</td>
				</tr>
				<tr >
					<td >申请人所属公司:</td>
					<td >${discardSell.companyName}</td>
					<td >是否费用化资产:</td>
					<td >
					
					<c:choose>
							<c:when test="${discardSell.isExpenseAssets=='1'}">
							是
							</c:when>
							<c:when test="${discardSell.isExpenseAssets=='0'}">
							否
							</c:when>
						</c:choose>
					</td>
					...
					<tr >
					<td >资产所属部门:</td>
					<td >${discardSell.subordinateDepartment}</td>
					<td >办公地点:</td>
					<td >${discardSell.officeLocation}</td>
				</tr>
				<tr>
					<td >资产所属公司:</td>
					<td >${discardSell.adscriptionCompany}</td>
					<td >平台:</td>
					<td >${discardSell.platform}</td>
				</tr>
			...
			<table class="table_list" style="margin-top: 20px;">
				...
				<c:forEach items="${discardSell.listGneralAssets}" var="gneralAsset" varStatus="status">
					<tr >
					...
					<td>${gneralAsset.usePeople} </td>
					<td>${gneralAsset.useDepartment} </td>
					<td>${gneralAsset.persentStorageLocation}</td>
					<td>${gneralAsset.manufacturer}</td>
					</tr>
				</c:forEach>
				<tr>
					<td>抬头文本说明</td>
