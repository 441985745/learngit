...
<body>
            ...
            <tr>
                ...
                <td>
                    <input name="costCenterCode" type="text" class="inputbtnRefView" onfocus="this.className='colorfocusRefView';"
                           onblur="this.className='inputbtnRefView';" style="width:120px;" value="${param.costCenterCode}">
                ...
                <td>
                    <input name="costCenterName" type="text" class="inputbtnRefView" onfocus="this.className='colorfocusRefView';"
                           onblur="this.className='inputbtnRefView';" style="width:120px;" value="${param.costCenterName}">
    ...
    <div>
        <table class="adminlist" cellspacing="0" rules="all" border="1" id="userTable" style="border-collapse:collapse;width:100%;">
            ...
            <tbody>
            <c:forEach items="${costCenterPager.records}" var="costCenter" varStatus="status">
                <tr>
    ...
    <div style="width: 100%;">
    <tag:paginate offset="${costCenterPager.offset}" total="${costCenterPager.total}"
                  steps="${costCenterPager.pageSize}" uri="${costCenterPager.uri}"
                  formId="costCenterForm" parentWindowId="${costCenterPager.parentWindowId}"/>
    </div>
...
<script type="text/javascript">
    ...
    (function () {
        ...
        $('#queryBtn').click(function () {
            window.location.href = "${dynamicURL}/costCenter/getCodeAndName?" + $('#costCenterForm').serialize();
        });
        ...
        function setParent(code, name, dialogId) {
            if (dialogId != '') {
                if (dg.iDG(dialogId)) {
                    dg.iWin(dialogId).fillCostCenter(code, name);
                    dg.cancel();
                } else {
                    alert('请先打开父窗口');
                }
