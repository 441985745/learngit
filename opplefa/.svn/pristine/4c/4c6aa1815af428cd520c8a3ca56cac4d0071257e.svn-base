...
<body>
		 ...
		 <table>
	 		<tr>
				...
				<td class="td_left"><input name="document" type="text" id="document"
					...
					<input type="hidden" value="${processAssets.commitType}" id="commitValue">
				</td>
		 ...
		 <tr>
	 		...
	 		<td class="td_left">
				<input  style="width: 110px;" class="td_input" value="${processAssets.assetDepartmentName }" readonly="readonly" name="assetDepartmentName" type="text"
					...
					<input  name="assetDepartmentCode" type="hidden" id="assetDepartmentCode" value="${processAssets.assetDepartmentCode }">
			</td>
			...
			<td class="td_right"><span class="span_color">*</span>组合资产责任人归属公司:</td>
	 		<td class="td_left">
	 				<select id="company" style="width: 163px;" class="td_input" onchange="getCostCenter()">
							...
							value="${processAssets.homeCompany}">${processAssets.homeCompany
							}</option> 
	 				</select>
	    	...
	    	<form action="" id="">
					<table id="Table8" border="0" cellpadding="3px" cellspacing="0"
						...
						<!-- 		<td align="right" class="tdtitle">管理编码:</td>
								...
								<td class="tdtext"><input name="persentCompany" type="text" id="persentCompany" class="colorblur"></td>
							</tr>
						</tbody>
					</table>
	...
	$(function() {
		$("#assetType option[value=${processAssets.assetType}]").attr(
				"selected", "selected");
		$("#storagePlace option[value='${processAssets.storagePlace}']").attr("selected", "selected");
		$("#officeLocation option[value='${processAssets.officeLocation}']").attr("selected", "selected");
	...
	function searchInfo(){
        ...
        $.ajax({
            ...
            success: function(data) {
				...
				for (var i = 0; i < data.length; i++) {  
					divHtml += "<tr align='center' value='"+data[i].id+"' id='tr"+data[i].id+"' class='bodyTr' style='white-space:nowrap;'>";
					divHtml += "<td id='k"+data[i].id+"' value='"+data[i].id+"' ><input type='checkbox' value='"+data[i].assetCode+","+data[i].id+","+data[i].companyCode+"' name='radioFrame' id='kuang"+data[i].id+"'></td>";
					divHtml += "<td class='tdpadding'>"+data[i].assetCode+"</td>";
					divHtml += "<td class='tdpadding'>"+data[i].assetName+"</td>";
					divHtml += "<td class='tdpadding'>"+data[i].putUnderManageCode+"</td>";
					divHtml += "<td class='tdpadding'>"+data[i].assetPerson+"</td>";
					divHtml += "<td class='tdpadding'>"+data[i].persentCompany+"</td>";
...
/* 		company = company.substring(0,company.length-1);
		...
		$("input[name='radio']").each(function(){
		  ...
		  $.ajax({
            type: "POST",
            url: "${dynamicURL}/processAssets/api/updateProcessAsset?"+$("#formId").serialize(),
            data: {"ids":assetIds,"date":date,"originalValueSum":originalValueSum,"assetNetValueSum":assetNetValueSum,"cause":cause},
            async:false,
            success: function(data) {
            	if (data.success) {
					alert("修改成功！");
	...
	/* $("#conservation").click(function(){
	   ...
	   function showUserDialog(){
		  var url = "${dynamicURL}/user/getUserList?parentWindowId=update";
		  var parentWindow=dg.curWin; 
