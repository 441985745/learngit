...
function GetViewHeight() {
    return document.getViewportHeight();
}
...
var downMsg = function (msgid, contentid, config) {
    ...
    this.moveUpDiv = function () {
        if (_this.offsetHeight > parseInt(_this.content.style.height, 10)) {
            ...
            _jsc.cookies.setCookie(_this.config.cookie, "show", _this.config.expiresDay); // ie6下才做，因为没有fixed属性 
            var isMSIE = !!(
            /*@cc_on!@*/
            0);
