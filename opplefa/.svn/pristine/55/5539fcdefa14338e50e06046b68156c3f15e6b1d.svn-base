...
<body>
    <form id="positionForm">
        <table cellpadding="1" bgcolor="#F9F9F9" class="adminlist">
            ...
            <tr>
                ...
                <td>
                    <input name="caddressCode" type="text" class="inputbtnRefView" onfocus="this.className='colorfocusRefView';"
                           onblur="this.className='inputbtnRefView';" style="width:120px;" value="${param.caddressCode}">
                ...
                <td>
                    <input name="caddressName" type="text" class="inputbtnRefView" onfocus="this.className='colorfocusRefView';"
                           onblur="this.className='inputbtnRefView';" style="width:120px;" value="${param.caddressName}">
    ...
    <div>
        <table class="adminlist" cellspacing="0" rules="all" border="1" id="userTable" style="border-collapse:collapse;width:100%;">
            ...
            <tbody>
            <c:forEach items="${workAddressPager.records}" var="workAdress" varStatus="status">
                <tr>
                    <td align="left">${workAdress.caddressCode}</td>
                    <td align="left">${workAdress.caddressName}</td>
    ...
    <div style="width: 100%">
    <tag:paginate offset="${workAddressPager.offset}" total="${workAddressPager.total}"
                  steps="${workAddressPager.pageSize}" uri="${workAddressPager.uri}"
                  formId="positionForm" parentWindowId="${workAddressPager.parentWindowId}"/>
    </div>
...
<script type="text/javascript">
    ...
    (function () {
        ...
        $('#queryBtn').click(function () {
            window.location.href = "${dynamicURL}/config/workAddress/getWorkAddressCode?" + $('#positionForm').serialize();
        });
        ...
        function setParent(code, name, dialogId) {
            if (dialogId != '') {
                if (dg.iDG(dialogId)) {
                    dg.iWin(dialogId).fillVehicleUser(code, name);
                    dg.cancel();
                } else {
                    alert('请先打开父窗口');
                }
