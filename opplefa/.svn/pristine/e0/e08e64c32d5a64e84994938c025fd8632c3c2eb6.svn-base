                ...
                <div>
                    ...
                    <table border="0" cellspacing="0" cellpadding="0" id="format">
                        ...
                        <tr>
                            ...
                            <td><input name="areaCode" type="text" value="${area.areaCode}" class="colorblur" readonly="readonly"/> </td>
                        </tr>
                        <tr>
                            <td class="tdtitle"><span>*</span>区域名称:</td>
                            <td><input name="areaName" type="text" value="${area.areaName}" class="colorblur" onblur="blur(this, '区域名称不能为空')"/> </td>
                        </tr>
                        <tr>
                            <td class="tdtitle">区域类型:</td>
                            <td><select name="areaType" class="colorblur" style="border: #cccccc 1px double;width: 60px;">
                                <option value="0" ${area.areaType == '0' ? "selected" : ""}>家居</option>
                                <option value="1" ${area.areaType == '1' ? "selected" : ""}>商用</option>
                            </select></td>
                        </tr>
                        <tr>
                            <td class="tdtitle">数据状态:</td>
                            <td><select name="status" class="colorblur" style="border: #cccccc 1px double;width: 60px;">
                                <option value="0" ${area.status == '0' ? "selected" : ""}>有效</option>
                                <option value="1" ${area.status == '1' ? "selected" : ""}>无效</option>
                            </select></td>
                        </tr>
                        </tbody>
                    </table>
...
<script type="text/javascript">
    ...
    $("#btnSave").click(function () {
        ...
        $.ajax({
            url : "${dynamicURL}/area/api/updateArea",
            type : "post",
            contentType : "application/json; charset=utf-8",
            data : JSON.stringify($("#areasUpdateForm").serializeObject()),
