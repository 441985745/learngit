                ...
                <form action="unit" id="unitForm">
                    <fieldset id="fset_ShipInportInfo">
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" id="t_ShipInportInfo"
                            ...
                            <tbody>
                            <tr>
                                <td align="right" class="tdtitle">单位编码:
                                </td>
                                <td class="tdtext">
                                    <input type="text" id="unitCode" value="${param.unitCode }" name="unitCode"
                                           class="colorblur">
                                </td>
                                 <td align="right" class="tdtitle">单位名称:
                                </td>
                                <td class="tdtext">
                                    <input type="text" id="unitName" value="${param.unitName }" name="unitName"
                                           class="colorblur">
                                </td>
                            </tr>
                            <tr>
                ...
                <div id="gridview" style="width: 1045px; height: 328px;">
                    <div>
                        <table class="adminlist" cellspacing="0" rules="all" border="1"
                            ...
                            <c:forEach items="${assetUnitPager.records}" var="asut" varStatus="status">
                                <tr style="white-space: nowrap;">
                                    <td align="center">
                                        <input type="checkbox" name="checkboxName" value="${asut.unitCode }">
                                    </td>
                                    <td>${status.index + 1}</td>
                                    <auth:authTag code="Edit">
                                        <td align="center">
                                            <a href="javascript:opUpdatedlg('updateAssetUnit','单位修改','${dynamicURL}/config/assetUnit/updateReady?flag=true&unitCode=${asut.unitCode }')">修改</a>
                                        </td>
                                    </auth:authTag>
                                    <auth:authTag code="View">
                                        <td align="center">
                                            <a href="javascript:opUpdatedlg('showAssetUnit','单位查看','${dynamicURL}/config/assetUnit/updateReady?flag=false&unitCode=${asut.unitCode }')">查看</a>
                                        </td>
                                    </auth:authTag>
                                    <td>${asut.unitCode}</td>
                                    <td>${asut.unitName}</td>
                                    <td>${asut.createBy}</td>
                                    <td><fmt:formatDate value="${asut.createDate}" pattern="yyyy-MM-dd"/></td>
                                    <td>${asut.updateBy}</td>
                                    <td><fmt:formatDate value="${asut.updateDate}" pattern="yyyy-MM-dd"/></td>
                                    <td>${asut.memo}</td>
                                    <td id="status${asut.unitCode}">${asut.status=='Y'?"有效":"无效"}</td>
                                </tr>
                            </c:forEach>
                            </tbody>
                        </table>
                        <tag:paginate offset="${assetUnitPager.offset}"
                                      total="${assetUnitPager.total}"
                                      steps="${assetUnitPager.pageSize}"
                                      uri="${assetUnitPager.uri}"
                                      formId="unitForm"/>
                    </div>
                </div>
            </td>
...
<jsp:include page="import_asset_unit_window.jsp"></jsp:include>
...
$(function(){
	...
	$("#btnQuery").click(function(){
	    searchData("${dynamicURL}/config/assetUnit/searchPagerList?" + $('#unitForm').serialize());
	});
    //导入和模版下载窗口
    var ew = new EasyWindow();
    ew.initEasyWindow("数据导入", "${dynamicURL}/config/assetUnit/importExcel");
    $('#btnImport').click(function () {
        ew.showEasyWindow();
    });
	 
    $("#btnAdd").click(function(){
    	var url = "${dynamicURL}/config/assetUnit/addReady";
    	addReady('addAssetUnit',url,"数据新增");
    });
    //删除功能
    $("#btnDelete").click(function(){
        if (confirm("确认删除？")) {
            deletes('checkboxName', "${dynamicURL}/config/assetUnit/delete", 'unitCodes');
    ...
    $("#btnExport").click(function (){
        if (window.confirm("确认要导出吗?")) {
            window.location.href = "${dynamicURL}/config/assetUnit/exportFile?"+$('#unitForm').serialize();
        }
    });
});
