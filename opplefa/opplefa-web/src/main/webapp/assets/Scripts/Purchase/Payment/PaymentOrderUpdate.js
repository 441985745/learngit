document.domain = 'opple.com';
	...
	function saveOrder(strDraft,isMail, isPhoneMessage, idea, activityId){
		...
		$.ajax({
	        ...
	        url: _dynamicURL+"/payment/api/updatePayment",
	...
	function checkAll(){
		...
		$("input[name='payCount']").each(function() {
			...
			if(payCount-(applyCount-paidCount)-oldCount>0){
				mess += "本次付款数量超过剩余可付数量;";
				flag = false;
				return true;
			}
			...
			if(paymentType=="验收款"){
				...
				var lastCount = $(this).parent().parent().find("#lastCount").val(); //上期付款数量
				var oldCount = $(this).parent().parent().find("#oldCount").val(); //新增时付款数量
				if(payCount-(checkCount-paidCount)-oldCount>0){
					mess += "本次付款数量超过(已验收数量-已付数量);";
					flag = false;
					return true;
				}
				if(payCount-(lastCount-paidCount)-oldCount>0&&$("#nper").val()!=1){
		...
		if(paymentType=="验收款"){
			$("#tbody_detail_detail").find('input[name^=gvdata]').each(function(){
				if($(this).attr('checked')){
					payPriceDetail += $(this).parent().parent().find("#payAmount").val();
	...
	$("#btnSave").click(function() {//提交
		var iamoney = $("#iamoney").val();
		var changeAmount = "Y";
		if(iamoney-realMoney == 0){
			changeAmount = "N";
		}
		if(ratioMoney==realMoney){
			var cdocument = $("#cdocument").val();
			if(checkAll()){
				showAgreeDialog(cdocument,77071,changeAmount);
			}
		}
		else{
	...
	function sumAll(){
		...
		$("input[id^='payCount']").each(function(){
			...
			if(paymentType=="验收款"){
				...
				$(this).parent().parent().find("#finalPrice").val((count*price-0)-((count*price-0)*lastPaymentRatio/100)-((count*price-0)*paymentRatio/100));//未付尾款金额    (应付金额-已付预付款金额-本次付款金额)
			}
			$(this).parent().parent().find("#payPrice").val((count*price-0)*paymentRatio/100);//本次付款金额
			$(this).parent().parent().find("#payPriceLocal").val(((count*price-0)*paymentRatio/100)*exchangeRate-0);//本次付款金额(本币)
		});
		//合计订单总价
