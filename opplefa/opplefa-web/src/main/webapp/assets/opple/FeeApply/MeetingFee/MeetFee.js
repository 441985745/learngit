...
function opAdddlg(id, statu, title) {
    var h = typeof (window.parent.parent.GetViewHeight()) == "undefined" ? 600 : window.parent.parent.GetViewHeight() - 20;
    var lurl = location.href.substring(0, location.href.lastIndexOf('/')) + "/Add.aspx?t=" + statu + "&id=" + id;
    dlg = new $.dialog({ id: 'add', title: title, cover: true, page: lurl, rang: true, width: 1000, height: h, btnBar: false, resize: true, maxBtn: true }).ShowDialog();
}
