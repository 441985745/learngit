...
$(document).ready(function () {
    ...
    $("#closeMessageBox").click(function () {
        if (confirm("如果沒有新的信息，此窗口將不会显示，确认关闭吗?")) {
            $("#messageBoxDIV").hide()
        }
    });
    GetOnLine();
            ...
            function (messages) {
                ...
                if (messFrom == "System") {
                    ...
                    if (messFrom != WorkerId) {
                        $(parent.parent.document.body).stopTime('controlled');
                        $(parent.parent.document.body).everyTime(1000, 'controlled', function (i) {
                            var f = i % 2;
                            if (f == 0) {
                                parent.parent.document.title = "【新消息】";
...
//                                tabl.title = '<span style="color:red">与' + messFrom + '1通话</span>';
                               //alert(tabl.title);
                                ...
                                parent.parent.document.title = "消息来自" + messFrom;
                                //$("#li_bottom_bar_" + messFrom).css({ color: "blue" });

                            }
                            if (i == 10) {
                                parent.parent.document.title = "YHGXBCC";
                                $(parent.parent.document.body).stopTime('controlled');
            ...
            function () {
                ...
                if (confirm("系统重新启动或者该帐号在其他地方登陆,是否重新登陆")) {
...
function PageInint()
{
  ...
  $("#ExprandDIV").click(function() {
   var top=$(this).offset().top;
   var left=$(this).offset().left;
   var height=$("#onlineListDIV").height();
   var width=$("#onlineListDIV").width();
   var newTop=top-height-10;
   var bigWidth=$(document).width();
   $("#onlineListDIV").css({top:newTop+"px",left:(left-width+5)+"px"});
   $("#onlineListDIV").toggle();
  });
  $("#TOSENDALL").click(function()
  {
    SendTo="ALL";
    $("#send_person").html("当前接收人:<span style='color:blue'>所有人</span>");
  });
  GetOnlineList();
...
function GetOnlineList()
{
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#online_span").html(onLineCount);
            $("#west").html(data.replace(onLineCount, ""));            
        }
    });
}
...
function CreateTabChat(WorkerId, obj) {
    //alert(WorkerId + "|" + obj);
