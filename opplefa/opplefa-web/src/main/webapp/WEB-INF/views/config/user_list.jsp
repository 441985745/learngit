    ...
    <script type="text/javascript">
        ...
        (function () {
            ...
            $('#queryBtn').click(function () {
            	window.location.href = '${dynamicURL}/user/getUserListInAttachConfig?' + $('#searchUserForm').serialize();
            });

            $('#userTable > tbody > tr').click(function() {
                setParent(this.children[0].innerText, this.children[1].innerText,dialogId, mark);
            });

            function setParent(code, name, dialogId, mark) {
            	if (dialogId != '') {
                    if (dg.iDG(dialogId)) {
                        dg.iWin(dialogId).fillVehicleUser(code, name, mark);
//                        dg.cancel();
                    } else {
                        alert('请先打开父窗口');
                    }
